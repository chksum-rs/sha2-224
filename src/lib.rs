...
//! let digest = sha2_224::async_chksum(file).await?;
//! assert_eq!(
//!     digest.to_hex_lowercase(),
//!     "90382cbfda2656313ad61fd74b32ddfa4bcc118f660bd4fba9228ced"
//! );
...
//! ```toml
//! [dependencies]
//! chksum-sha2-224 = { version = "0.1.0", features = ["reader", "writer"] }
//! ```
//!
//! Alternatively, you can use the [`cargo add`](https://doc.rust-lang.org/cargo/commands/cargo-add.html) subcommand:
//!
...
/// # async fn wrapper() {
/// let data = b"example data";
/// if let Ok(digest) = sha2_224::async_chksum(data).await {
///     assert_eq!(
///         digest.to_hex_lowercase(),
///         "90382cbfda2656313ad61fd74b32ddfa4bcc118f660bd4fba9228ced"
///     );
...
pub async fn async_chksum(data: impl core::AsyncChksumable) -> Result<Digest> {
    core::async_chksum::<SHA2_224>(data).await
}
